...
const TZ="America/Toronto"; const state={units:["P334","FB334"],selected:"P334",calls:[]};
const $=id=>document.getElementById(id), tabs=$("tabs"), daysEl=$("days"), unitStat=$("unitStat"), shiftStat=$("shiftStat"), totalStat=$("totalStat"), statusEl=$("status"), daysSelect=$("daysSelect"), unitInput=$("unitInput");
...
function torontoToday(){const p=parts();return `${p.year}-${p.month}-${p.day}`;}
...
if("serviceWorker" in navigator) navigator.serviceWorker.register("/sw.js").catch(()=>{});renderTabs();loadCalls();
